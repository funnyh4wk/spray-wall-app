...
let currentClimbersData = [], profileBackTarget = 'home'; 
...
function getPoints(history) {
    ...
    history.forEach(a => {
        const g = a.grade;
        if (!g) return;
        const idx = ALL_GRADES.indexOf(g);
        if (idx === -1) return;
...
async function googleSignIn() {
    try {
        ...
        if (!profile || profile.detail) {
            ...
            profile = { 
                user_id: userCred.user.uid, email: userCred.user.email, name: displayName, 
                first_name: displayName.split(" ")[0] || "", last_name: displayName.split(" ").slice(1).join(" ") || "", 
                bio: "Bouldering Enthusiast", avatar_url: userCred.user.photoURL || "", 
                is_global_admin: false, can_create_gyms: false 
            };
            const res = await apiCall('/api/db/save', { path: `users/${userCred.user.uid}`, payload: profile });
            if (!res) return;
        }
        ...
        profile.ascents_history = normArr(await apiCall('/api/db/get', { path: `user_ascents/${userCred.user.uid}` }));
        localStorage.setItem('user_profile', JSON.stringify(profile)); 
        loadHomeView(); showView('view-home'); showNotify("Welcome!");
    } catch (error) { showNotify(error.message.replace("Firebase: ", ""), true); }
}

function logout() { 
    auth.signOut(); localStorage.removeItem('user_profile'); 
    document.getElementById('auth-email').value = ''; document.getElementById('auth-password').value = ''; document.getElementById('auth-password-confirm').value = '';
    showView('view-auth'); 
}
...
function renderGradeChart(containerId, gradesArray) {
    ...
    Object.keys(counts).sort((a,b) => ALL_GRADES.indexOf(a) - ALL_GRADES.indexOf(b)).forEach(grade => {
        ...
        <div class="flex flex-col justify-end items-center flex-1 min-w-[32px] h-full mx-1">
            <span class="text-[10px] text-gray-400 mb-1">${counts[grade]}</span>
            <div class="w-full bg-blue-500 rounded-t-sm transition-all duration-500 shadow-md" style="height: ${heightPercent}%"></div>
...
async function loadFriendsView() {
    ...
    for(let fId in friends) {
        const u = allUsers[fId]; if(!u) continue;
...
async function loadGymsView() {
    ...
    for(let gId in gyms) {
        const g = gyms[gId]; if(!g || !g.id) continue;
...
function openRouteDetail(b) {
    ...
    if (ascent) {
        actionsDiv.innerHTML = `
            <button onclick="navigate('gym-routes')" class="bg-gray-700 hover:bg-gray-600 px-4 py-3 rounded font-bold text-sm transition-colors flex-1 shadow-md">Back</button>
            <button onclick="removeAscent()" class="bg-green-600 hover:bg-red-500 px-4 py-3 rounded font-bold text-sm transition-colors flex-[2] shadow-md">✓ Completed</button>
        `;
    } else {
        actionsDiv.innerHTML = `
            <button onclick="navigate('gym-routes')" class="bg-gray-700 hover:bg-gray-600 px-4 py-3 rounded font-bold text-sm transition-colors flex-1 shadow-md">Back</button>
            <button onclick="startCompleteRoute()" class="bg-gray-600 hover:bg-gray-500 px-4 py-3 rounded font-bold text-sm transition-colors text-gray-300 flex-[2] shadow-md">Complete Route</button>
        `;
    }
...
async function confirmCompleteRoute(isOfficial = false) {
    ...
    showNotify("Ascent logged!"); openRouteDetail(activeBoulder);
}
...
async function loadWallPhoto(input) {
    const file = input.files[0]; if(!file) return; showNotify("Uploading wall photo..."); const formData = new FormData(); formData.append("file", file);
    let token = await getAuthToken();
    const res = await (await fetch('/api/upload', { method: 'POST', headers: { 'Authorization': `Bearer ${token}` }, body: formData })).json();
    if(res.status === "ok") { document.getElementById('wizard-wall-img').src = res.url; document.getElementById('detail-wall-img').src = res.url; wizardStep(2); showNotify("Photo uploaded!"); } else showNotify("Upload failed", true);
}
...
window.onload = () => {
    ...
    if(profileStr) { 
        ...
        if (profile.detail || !profile.user_id) { localStorage.removeItem('user_profile'); showView('view-auth'); return; }
